// backend/models/outreachReportModel.js
const mongoose = require('mongoose');

const outreachReportSchema = mongoose.Schema(
    {
        // This report is primarily managed by the Assistant National Coordinator Secondary School Outreach.
        // It's linked to the submitting user rather than a specific fellowship or zone directly,
        // reflecting its national scope for this specific outreach.
        reportingMonth: {
        ...
        submittedBy: {
            ...
            ref: 'User',
            required: true, // Likely the Asst. National Coordinator or a designated outreach officer
        },
        status: {
            type: String,
        ...
        approvedBy: {
            type: mongoose.Schema.Types.ObjectId,
            ref: 'User', // Likely National Coordinator or Super Admin
        },
        approvalDate: {
            type: Date,
        },
...
// Compound unique index: One outreach report per submitting user per calendar month
outreachReportSchema.index({ submittedBy: 1, reportingMonth: 1 }, { unique: true });

// Pre-save hook to calculate summary fields from `detailsOfVisits`
outreachReportSchema.pre('save', function (next) {
...
const OutreachReport = mongoose.model('OutreachReport', outreachReportSchema);

module.exports = OutreachReport;